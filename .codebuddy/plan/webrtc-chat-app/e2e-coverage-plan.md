...
| G21 | No "Delete conversation" affordance on the conversation row | `SidebarConversationMenu` exposes only Pin / Mute / Archive items. There is no per-row delete (or "remove from list") action anywhere in the UI, and no corresponding `AppState` method either — `archived` is the closest soft-delete primitive. Plan §3 P2-4 originally listed a "delete conversation" assertion; the test was dropped pending a feature decision (hard delete vs. archive-only) and the supporting UI affordance + IDB tombstone migration. | P2-4 |
...
| G27 | Theater video source round-trip needs a codec-compatible test fixture | Plan §3 P2-5 originally listed an "owner selects local video → frame plays" assertion. Implementing it deterministically requires (a) a tiny, codec-compatible video file under `e2e/assets/` (e.g. an H.264 / VP9 ≤1 KB stub), and (b) a readiness oracle on `<video>` that survives Chromium's lazy decode (e.g. polling `videoWidth > 0` after `play()`). The first is non-trivial — the smallest valid mp4/webm is ~1 KB but not all Chromium builds accept every codec subset. Furthermore, the player gates `<video>` mounting on the `loadedmetadata` event firing, so the element doesn't even appear in the DOM until a real video URL resolves. The P2-5 spec now covers the happy-path picker surface (owner-side `theater-page` mounts, source-picker is visible) plus the URL-validation alert path (`ftp://example.test` is rejected with a visible `role="alert"`). The remaining video-source / viewer-join / cross-peer danmaku assertions land once the fixture ships. | P2-5 |
| G28 | Nickname change is not persisted in the global User table | `server/ws/room/handle_nickname_change` only validates and broadcasts the new nickname to current room members, plus writes it into the room-scoped `MemberInfo`. It never updates the `User` row in `server::auth`. Consequence: after a client `NicknameChange` round-trip, a page reload triggers `try_recover_auth` → `AuthSuccess` from the server with `nickname = username`, and `handle_auth_success` overwrites `localStorage["auth_nickname"]` with that stale value. The G25-resolved `NicknameEditor` therefore round-trips correctly within a session but is wiped on reload. The fix is server-side: persist the new nickname on the `User` struct inside `handle_nickname_change`, and emit the canonical value back in subsequent `AuthSuccess` frames. The corresponding cross-reload assertion in `profile.spec.ts:G25` is currently scoped to the in-session contract and will be expanded once the server-side persistence ships. | P2-7 |
...
- [~] P2-4 conv-list-management.spec.ts — pin / mute / archive +
  search filter (G20 resolved); "delete conversation" deferred to
  G21 (no delete affordance exists in the row context menu).
- [~] P2-5 theater.spec.ts — creator/source-picker + in-room chat
  round-trip + URL-picker validation (G27 partial). The full
  video-source playback / viewer-join / cross-peer danmaku
  assertions stay deferred under G27 until a codec-compatible
  test video fixture lands.
- [x] P2-6 settings.spec.ts
- [~] P2-7 profile.spec.ts — block / unblock / blacklist
  persistence + in-session nickname edit (G25 resolved). Cross-
  reload nickname persistence stays deferred under G28 (server
  does not write the new nickname to the User table); "avatar
  upload" deferred to G26 (no UI affordance + no protocol field).
