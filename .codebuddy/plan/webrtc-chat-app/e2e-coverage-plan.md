...
| G20 | Sidebar search input is rendered but has no behaviour | `components/sidebar/mod.rs` mounts a `<input type="search" class="sidebar-search-input">` in the sidebar header but the element binds neither `prop:value` nor `on:input` to any signal. As a result typing into it produces no filtering of the conversation lists, no suggestions, and no other observable side effect. Plan §3 P2-4 originally listed a "search filter" assertion; that test was dropped in this wave because the surface is decorative-only. A future ticket should wire the input to a `RwSignal<String>` and have the three sidebar sections (`pinned_memo`, `active_memo`, `archived_memo`) consume it as a case-insensitive `display_name` filter, mirroring the existing `ForwardModal` filter logic. | P2-4 |
...
- [~] P2-7 profile.spec.ts — block / unblock / blacklist
  ...
  (`NicknameEditor` component never mounted); "avatar upload"
  deferred to G26 (no UI affordance + no protocol field).
