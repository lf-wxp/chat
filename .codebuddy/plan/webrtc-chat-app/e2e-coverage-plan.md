...
| G17 | All `position: fixed` modals were trapped inside `.app` instead of covering the viewport | The authenticated shell's root `<div class="app …">` carries `view-transition-name: app-root` (`main.css:75`). Per the CSS View-Transitions spec, an element with a `view-transition-name` establishes a new containing block for its `position: fixed` (and `position: absolute`) descendants — the same effect `transform`, `filter`, `perspective`, `will-change`, and `contain: paint` all have. Combined with the `.app flex overflow-hidden` layout class, **every `position: fixed` modal rendered inside `.app` resolved its `inset: 0` against `.app` rather than the viewport, then got clipped to whatever sub-region of `.app` was visible**. The most visible symptom was `InviteMemberModal` rendering as a small rectangle inside the sidebar's member panel; `RoomSettingsModal`, `MemberHistoryPanel`, `ConfirmDialog`, `MuteDurationPicker`, `ForwardModal`, `AnnouncementPanel` were all latently broken the same way. Fixed in two parts: (a) Promote `InviteMemberModal` to a parameter-less global component opened by `GlobalRoomModalState::invite_target: RwSignal<Option<RoomInfo>>`, mirroring `CreateRoomModal` / `PasswordPromptModal`. (b) **The actual fix** — split the authenticated shell into two siblings inside `<Show>`: a layout `<div class="app …">` and a sibling fragment hosting the overlays (`ToastContainer`, `ModalManager`, `CallOverlay`, `IncomingInviteModal`, `DebugPanel`). With the overlays no longer descendants of the view-transition element, `position: fixed` resolves against the viewport again and every modal — including the ones still triggered locally — covers the full screen. | dialog-arch |
| G18 | Several room-domain modals are still triggered with locally-scoped `RwSignal<bool>` flags rather than via `GlobalRoomModalState` | `RoomSettingsModal`, `MemberHistoryPanel`, `MuteDurationPicker` (single + batch), `ConfirmDialog` (multiple sites), `ForwardModal`, and `AnnouncementPanel` are all still rendered inside their trigger components (e.g. `member_list.rs`, `room_settings_modal.rs`, `chat_view/view_root.rs`) and gated by component-local `RwSignal<bool>` flags. After G17 they no longer have a clipping bug — the viewport-covering behaviour is correct — but the architecture is inconsistent with `CreateRoomModal` / `PasswordPromptModal` / `InviteMemberModal` / `IncomingRoomInviteModal`, all of which live inside `ModalManager` and open via `GlobalRoomModalState` signals. A future cleanup ticket should fold the remaining six modals into the same shape: add `room_settings_target`, `member_history_target`, `mute_target` (single + batch), `forward_source`, `announcement_target`, and `confirm_request` fields to `GlobalRoomModalState`; render every modal inside `ModalManager`; reduce trigger sites to a single `state.<field>.set(Some(...))` call. The benefit is consistency, easier testing (one place to mock open/close), and protection against any future re-introduction of the G17 containing-block bug. | dialog-arch |
