...
/// password-prompt and invite-member modals.
///
/// All room-related modals are rendered at the app root inside
/// `ModalManager` so that an ancestor's `overflow: hidden` (e.g. the
/// sidebar member panel) cannot clip them. Triggers anywhere in the
/// tree mutate the signals below to open/close each modal — they never
